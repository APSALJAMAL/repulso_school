    ...
    if (req.user.isAdmin && !!school) {
      ...
      school = {
        ...
        _count: {
          ...
          admins: allMembers.filter((member) => member.role == Role.ADMIN).length,
          superadmins: allMembers.filter((member) => member.role == Role.SUPER_ADMIN).length,
        },
      };
    }
...
export const getAllMarks = async (req: Request, res: Response) => {
  try {
    const marks = await db.markOnRow.findMany({
      include: {
        student: true,
        row: {
          include: {
            subject: true, // ✅ This is valid
          },
        },
...
export async function getUserById(schoolId: string, userId: string) {
  ...
  // Optionally flatten the role out, if you want just one role:
  const role = user.schools.length > 0 ? user.schools[0].role : null;

  return { ...user, role };
}


export const getTeachersInSchool = async (req: Request, res: Response) => {
  try {
